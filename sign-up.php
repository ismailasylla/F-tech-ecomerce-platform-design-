
    

<div class="container" style="width: 350px;">
		<div class="row">
			<div class="panel panel-primary">
				<div class="panel-body">
					<form method="POST" action="./api/signup.php" role="form">
						<div class="form-group">
							<h2>Cree un compte</h2>
						</div>
						<div class="form-group">
							<label class="control-label" for="firstname">Full Name</label>
							<input id="signupName" type="text" maxlength="50" class="form-control" name="fullname">
						</div>
						<div class="form-group">
							<label class="control-label" for="signupEmail">Email</label>
							<input id="signupEmail" type="email" maxlength="50" class="form-control" name="email">
						</div>
						<div class="form-group">
							<label class="control-label" for="signupPassword">Password</label>
							<input id="signupPassword" type="password" maxlength="25" class="form-control" placeholder="at least 6 characters" length="40" name="password">
						</div>
						<div class="form-group">
							<button id="signupSubmit" type="submit" class="btn btn-info btn-block" name="submit">Create your account</button>
						</div>
						<p class="form-group">By creating an account, you agree to our <a href="#">Terms of Use</a> and our <a href="#">Privacy Policy</a>.</p>
						<hr>
						<p></p>Already have an account? <a href="#">Sign in</a></p>
					</form>
				</div>
			</div>
		</div>
	</div>

	<hr>

<div class="a-section a-spacing-top-extra-large" style="text-align:center;">
        

<div class="a-divider a-divider-section"><div class="a-divider-inner"></div></div>
<div class="a-section a-spacing-small a-text-center a-size-mini">
  <span class="auth-footer-seperator"></span>
  
    
    <a class="a-link-normal" target="_blank" href="/gp/help/customer/display.html/ref=ap_desktop_footer_cou?ie=UTF8&amp;nodeId=548524">
      Conditions d'utilisation
    </a>
    <span class="auth-footer-seperator"></span>
  
    
    <a class="a-link-normal" target="_blank" href="/gp/help/customer/display.html/ref=ap_desktop_footer_privacy_notice?ie=UTF8&amp;nodeId=3329781">
      Protection de vos informations personnelles
    </a>
    <span class="auth-footer-seperator"></span>
  
    
    <a class="a-link-normal" target="_blank" href="/help">
      Aide
    </a>
    <span class="auth-footer-seperator"></span>
  
</div>

<div class="a-section a-spacing-none a-text-center">
  <span class="a-size-mini a-color-secondary">
    © 2015-2017, F-Tech.com, Inc. ou ses filiales.
  </span>
</div>

      </div>








































<!--<div class="container"style="text-align:center;">
        <div class="row">
        <h3>User Sign-up</h3>
        </div>


<div class="row">
    <p>Welcome Please create your new Account by sign-up</p><br><br>

    <form action="" method="POST" class="form-horizontal">

        <div class="row form-group">
    
                <label for="User name">Name</label>
                    <input type="text" name="name" placeholder="Enter your User Name"><br><br>
                <label for="email">Email</label>
                    <input type="text" name="email" placeholder="Enter your Email Address"><br><br>
                <label for="password">Password</label>
                    <input type="text" name="password" placeholder="Enter your password"><br><br>
                    <input type="submit" name="psubmit" class=" btn btn-primary ">
        
        </div>
    </form>


</div>



</div>-->